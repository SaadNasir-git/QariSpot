...
export async function POST(request: NextRequest) {
    return tryCatchBlock(async () => {
        ...
        const [surah] = await conn.query<RowDataPacket[]>('SELECT * FROM surah WHERE qariId=? url IS NOT NULL ORDER BY surahNo ASC LIMIT ? OFFSET ?', [Number(qariId), 10, offset ? Number(offset) : 0])
